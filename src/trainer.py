...
class TorchTrainer:
    ...
    def train(
        ...
        for epoch in range(n_epoch):
            ...
            wandb.log({"val/loss": _, "val/f1": test_f1, "val/test_acc": test_acc})
            ...
            if self.sweep:
                self.scheduler.step(best_test_f1)
            else:
                self.scheduler.step()
        if best_test_f1 > 0.75:
            os.rename(
                "/opt/ml/code/exp/sweep/best.ts",
                f"/opt/ml/code/exp/sweep/{best_test_f1}.ts",
            )
