...
def custom_aug(dataset: str = "CIFAR10", img_size: float = 32) -> transforms.Compose:
    """Simple data augmentation rule for training CIFAR100."""
    return transforms.Compose(
        [
            SquarePad(),
            SequentialAugmentation([("Sharpness", 0.2, 0.1)]),
            SequentialAugmentation([("Color", 0.2, 0.1)]),
            SequentialAugmentation([("Brightness", 0.2, 0.1)]),
            transforms.Resize((int(img_size * 1.2), int(img_size * 1.2))),
            transforms.RandomResizedCrop(
                size=img_size, ratio=(0.75, 1.0, 1.3333333333333333)
            ),
            SequentialAugmentation([("Cutout", 0.2, 0.1)]),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
            transforms.Normalize(
                DATASET_NORMALIZE_INFO[dataset]["MEAN"],
